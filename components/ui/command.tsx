...
function Command({
  ...
  return (
    <CommandPrimitive
      data-slot="command"
      className={cn(
        "bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-50 flex w-full flex-col rounded-md",
        className
      )}
      {...props}
    />
...
function CommandInput({
  ...
  return (
    <div
      ...
      className="flex h-9 items-center gap-2 border-b border-zinc-200 dark:border-zinc-700 px-3"
    >
      <SearchIcon className="size-4 shrink-0 opacity-50" />
      <CommandPrimitive.Input
        data-slot="command-input"
        className={cn(
          "placeholder:text-zinc-400 flex h-10 w-full rounded-md bg-transparent py-3 text-sm outline-none focus:outline-none focus-visible:outline-none focus-visible:ring-0 focus-visible:shadow-none disabled:cursor-not-allowed disabled:opacity-50",
          className
        )}
...
function CommandList({
  ...
  return (
    <CommandPrimitive.List
      ...
      onWheel={(e) => {
        // Ensure wheel events propagate for scrolling
        e.stopPropagation()
      }}
      {...props}
    />
